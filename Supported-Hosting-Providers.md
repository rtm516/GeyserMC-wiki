...
* [Server.pro](https://server.pro) (Get Geyser as a plugin. Use the same port as your Java server for the Bedrock port in your config and connect with that port; enable the `clone-remote-port` option if using the free plan; leave the IP as `0.0.0.0` and/or `auto`. Paid gameserver plans will only work if you use the same port as the Java Edition server due to the DDoS protection, however this issue is absent on all VPS plans.)
